...
namespace BovineLabs.Event.Containers
{
    ...
    /// </summary>
    public unsafe partial struct UnsafeEventStreamNew : INativeDisposable, IEquatable<UnsafeEventStreamNew>
    {
        [NativeDisableUnsafePtrRestriction]
        private UnsafeEventStreamBlockDataNew* m_Block;

        private Allocator m_Allocator;

        public UnsafeEventStreamNew(int foreachCount, Allocator allocator)
        {
            AllocateBlock(out this, allocator);
            this.AllocateForEach(foreachCount);
        }

        internal static void AllocateBlock(out UnsafeEventStreamNew stream, Allocator allocator)
        {
            int blockCount = JobsUtility.MaxJobThreadCount;

            int allocationSize = sizeof(UnsafeEventStreamBlockDataNew) + (sizeof(UnsafeEventStreamBlockNew*) * blockCount);
            byte* buffer = (byte*)UnsafeUtility.Malloc(allocationSize, 16, allocator);
            UnsafeUtility.MemClear(buffer, allocationSize);

            var block = (UnsafeEventStreamBlockDataNew*)buffer;
            ...
            block->Blocks = (UnsafeEventStreamBlockNew**)(buffer + sizeof(UnsafeEventStreamBlockDataNew));
        ...
        {
            long allocationSize = sizeof(UnsafeEventStreamRangeNew) * forEachCount;
            long allocationThreadSize = sizeof(UnsafeEventStreamThreadRangeNew) * forEachCount;
            m_Block->Ranges = (UnsafeEventStreamRangeNew*)UnsafeUtility.Malloc(allocationSize, 16, this.m_Allocator);
            m_Block->ThreadRanges = (UnsafeEventStreamThreadRangeNew*)UnsafeUtility.Malloc(allocationThreadSize, 16, this.m_Allocator); // todo conditional
        ...
        private void Deallocate()
        {
            ...
            {
                UnsafeEventStreamBlockNew* block = m_Block->Blocks[i];
                while (block != null)
                {
                    UnsafeEventStreamBlockNew* next = block->Next;
        ...
        {
            public UnsafeEventStreamNew Container;
        ...
        public bool Equals(UnsafeEventStreamNew other)
        {
